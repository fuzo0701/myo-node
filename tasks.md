...
1. **세션 JSONL 뷰어** (12.1 목록, 12.2 상세) — Low~Medium
2. **슬래시 명령 통합** (18.1 팔레트 확장) — Low
3. **Plan/Todo 뷰어** (19.1 플랜, 19.2 Todo) — Low
4. **세션 이어가기** (12.4 `--resume`) — Low
5. **글로벌 입력 히스토리** (20.1 크로스세션) — Low
6. **커스텀 명령 레지스트리** (21.1 뷰어) — Low
7. **키바인딩 뷰어** (16.1 목록) — Low
8. **인증 상태 표시** (17.1 OAuth 상태) — Low

### Phase 3 - 패키지 & 확장
> 외부 패키지 설치/관리 + 기존 기능 고도화

1. **플러그인 마켓플레이스** (14.1 브라우저, 14.2 설치) — Medium
